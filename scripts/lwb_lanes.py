...
# Segments that must NOT count as evidence of a shared session/dispatch
# token even when they clear MIN_SHARED_SEGMENT_LENGTH, because they name
# the product/repo itself rather than a session: every valid identity in
# this repo's history contains "buildcraft" and/or "leapware" somewhere
# (it is this repo's own name), so two wholly unrelated, genuinely
# independent reviewer/author ids were guaranteed to collide on that
# segment alone. Found by direct measurement, not guessed: PR #38's own
# false-positive is exactly this -- "buildcraft" embedded in both
# reviewer_id and commit_author_id tripped the collision check with no
# actual shared session behind it. Lowercased comparison, since segments
# are compared case-sensitively by `_id_segments`/`_shared_long_segment`
# but an id author should not be able to dodge the stoplist by
# capitalizing "BuildCraft".
...
def _id_segments(value: str) -> set[str]:
    ...
    trailing '-YYYY-MM-DD' date stripped first, and with any segment in
    IGNORED_ID_SEGMENTS (case-insensitive) filtered out, since those name
    the product/repo itself rather than a session and appear in every
    valid id by construction (see IGNORED_ID_SEGMENTS)."""
    m = _DATE_SUFFIX_RE.search(value)
...
def _shared_long_segment(id_a: str, id_b: str) -> Optional[str]:
    ...
    and product-name segments excluded, see _id_segments) and at least
    MIN_SHARED_SEGMENT_LENGTH characters long, or None if there is no such
    segment. Deterministic when more than one qualifies: the shortest,
